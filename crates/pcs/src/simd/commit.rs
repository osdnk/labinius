...
//! a Montgomery slot product would cost. Nothing but the sum is ever needed, and
//! `sum_i A_i[j] W_i[j]` is congruent mod q whatever representatives the transform leaves, so the
//! only question is overflow.
//!
//! # The exact fold-back
//!
...
//!
//! The first design was the VNNI form of the same product, one `vpdpwssd` per slot, which leaves
//! 16 i32 lanes per slot: 648 vectors, 41 KB, read and written once per slot per batch. That does
//! not fit L1 next to the A stream — measured 85 cycles per ring element against a 40-cycle uop
//! floor. Folding the 16 lanes of a slot down to 8 and packing two slots into one vector cuts the
//! accumulator to 21.5 KB and its traffic by a third for the same uop count (two `vpmaddwd` and
//! two `vshufi64x2` per two slots instead of two `vpdpwssd`, ALU work in place of loads):
//! 85 -> 58 cycles per ring element, and 58 is what the commitment pays. The fused add of
//! `vpdpwssd` only pays while the accumulator keeps the product's lane layout, and that layout is
//! exactly what is too big; once the products are shuffled before they meet the accumulator the
//! separate `vpaddd` is free, so nothing in this module emits a VNNI instruction.
//!
